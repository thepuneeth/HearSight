import SwiftUI

struct CleanArrivalView: View {
    @ObservedObject var viewModel: WalkthroughViewModel
    let onSave: () -> Void
    let onDone: () -> Void

    @Environment(\.colorScheme) private var colorScheme
    @State private var didSave = false

    var body: some View {
        let data = CleanFlowBriefing.resolved(from: viewModel)

        CleanStickyBottomContainer {
            CleanScreenHeader(
                title: "Arrival Assistance",
                subtitle: "Confirm the entrance before going inside.",
                eyebrow: data.destinationName,
                systemImage: "mappin.circle.fill"
            )

            CleanVoiceHint(text: "Say how your trip was, then say a note — or use the buttons.")

            if viewModel.arrivalVoiceMode != .off {
                CleanCard(title: "Listening", systemImage: "mic.fill") {
                    VStack(alignment: .leading, spacing: HearSightTheme.Spacing.xs) {
                        Text(listeningPromptLabel)
                            .font(.body.weight(.semibold))
                            .foregroundStyle(.primary)
                        Text(viewModel.lastHeardText.isEmpty ? "…waiting for your voice" : "Heard: \(viewModel.lastHeardText)")
                            .font(.callout)
                            .foregroundStyle(.secondary)
                            .accessibilityHidden(true)
                    }
                }
            }

            CleanCueCard(
                title: "Correct entrance",
                cue: data.entranceCue,
                systemImage: "door.left.hand.open",
                onHear: { viewModel.speakImmediate(data.entranceCue) }
            )

            CleanLandmarkChainCard(landmarks: data.landmarkChain)

            HStack(spacing: HearSightTheme.Spacing.md) {
                CleanIconButton(
                    title: viewModel.isSpeaking ? "Speaking..." : "Repeat cues",
                    systemImage: viewModel.isSpeaking ? "speaker.wave.3.fill" : "speaker.wave.2.fill",
                    accessibilityLabel: "Repeat arrival cues",
                    accessibilityHint: "Repeats the entrance and landmark cues."
                ) {
                    speakArrivalCues(data)
                }

                CleanIconButton(
                    title: "Need help",
                    systemImage: "hand.raised.fill",
                    isProminent: true,
                    accessibilityLabel: "Need help",
                    accessibilityHint: "Repeats the entrance and landmark guidance."
                ) {
                    speakArrivalCues(data)
                }
            }

            if didSave {
                CleanCard(title: "Saved", systemImage: "checkmark.circle.fill") {
                    Text("Next time, HearSight can use this note to make the arrival easier.")
                        .font(.body.weight(.semibold))
                        .foregroundStyle(.primary)
                        .fixedSize(horizontal: false, vertical: true)
                }
            }

            CleanConfidencePicker(selection: $viewModel.confidenceLevel)

            CleanCard(title: "Note to future self", systemImage: "square.and.pencil") {
                TextField("Add a short note", text: $viewModel.futureSelfNote, axis: .vertical)
                    .font(.body.weight(.medium))
                    .lineLimit(3...5)
                    .padding(HearSightTheme.Spacing.md)
                    .frame(minHeight: 112, alignment: .topLeading)
                    .background(HearSightTheme.insetPanel(colorScheme))
                    .clipShape(RoundedRectangle(cornerRadius: HearSightTheme.Radius.md, style: .continuous))
                    .accessibilityLabel("Note to future self")
                    .accessibilityHint("Saves a short note for a future visit.")
            }
        } bottomAction: {
            CleanPrimaryButton(
                title: didSave ? "Done" : "Save Arrival Confidence",
                systemImage: didSave ? "house.fill" : "checkmark.circle.fill",
                accessibilityLabel: didSave ? "Done" : "Save Confidence",
                accessibilityHint: didSave ? "Returns to the start screen." : "Saves your confidence level and note."
            ) {
                if didSave {
                    onDone()
                } else {
                    saveArrivalConfidence()
                }
            }
        }
        .onAppear {
            if viewModel.confidenceLevel == nil {
                viewModel.confidenceLevel = .okay
            }
            startConfidenceQuestion(destinationName: data.destinationName)
        }
        .onDisappear {
            viewModel.arrivalVoiceMode = .off
        }
        .onChange(of: viewModel.pendingConfidenceLevel) { _, level in
            guard let level else { return }
            viewModel.pendingConfidenceLevel = nil
            viewModel.confidenceLevel = level
            onSave()
            didSave = true
            // Confirm the answer out loud, then ask for a note by voice.
            listen(.note, saying: "You said \(level.title). Would you like to add a note for next time? Say your note now, or say no.")
        }
        .onChange(of: viewModel.capturedArrivalNote) { _, note in
            guard let note, !note.isEmpty else { return }
            viewModel.capturedArrivalNote = nil

            if isSkipWord(note) {
                listen(.nextTrip, saying: "No note saved. Ready for your next trip? Say yes or no.")
            } else {
                viewModel.futureSelfNote = note
                onSave()
                listen(.nextTrip, saying: "Your note is: \(note). Ready for your next trip? Say yes or no.")
            }
        }
        .onChange(of: viewModel.pendingNextTripAnswer) { _, answer in
            guard let answer else { return }
            viewModel.pendingNextTripAnswer = nil
            viewModel.arrivalVoiceMode = .off
            if answer {
                viewModel.speakImmediate("Great. Returning home for your next trip.")
                onDone()
            } else {
                viewModel.speakImmediate("Okay. You can stay here. Press Done whenever you are ready.")
            }
        }
    }

    private func startConfidenceQuestion(destinationName: String) {
        listen(.confidence, saying: "You have arrived near \(destinationName). Use your normal mobility tools and surroundings to confirm the entrance. How was your trip? You can say very unsure, unsure, okay, confident, or very confident.")
    }

    // Set the listening mode and speak the prompt. The recognizer restarts cleanly after the
    // prompt finishes (see reviveGuidanceCommandsAfterSpeech) so the answer is heard.
    private func listen(_ mode: WalkthroughViewModel.ArrivalVoiceMode, saying prompt: String) {
        viewModel.arrivalVoiceMode = mode
        viewModel.startGuidanceVoiceCommands()
        viewModel.speakImmediate(prompt)
    }

    private var listeningPromptLabel: String {
        switch viewModel.arrivalVoiceMode {
        case .confidence: return "Say how your trip was: very unsure, unsure, okay, confident, or very confident."
        case .note: return "Say a note, or say no."
        case .nextTrip: return "Say yes or no."
        case .off: return ""
        }
    }

    private func isSkipWord(_ note: String) -> Bool {
        let n = note.lowercased().trimmingCharacters(in: .whitespacesAndNewlines)
        return ["no", "nope", "nah", "skip", "none", "nothing", "no thanks", "no note"].contains(n)
    }

    private func speakArrivalCues(_ data: CleanFlowBriefing) {
        let cue = [
            data.arrivalFocus,
            data.entranceCue,
            "Landmarks: \(data.landmarkChain.joined(separator: ", "))."
        ]
        .joined(separator: " ")
        viewModel.speakImmediate("Repeating arrival cues. \(cue)")
    }

    private func saveArrivalConfidence() {
        if viewModel.confidenceLevel == nil {
            viewModel.confidenceLevel = .okay
        }
        onSave()
        didSave = true
        viewModel.speakAccessibilityPrompt("Arrival confidence saved.")
    }
}

#Preview {
    CleanArrivalView(viewModel: WalkthroughViewModel(), onSave: {}, onDone: {})
}
