...
struct CleanConfirmDestinationView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            // The destination fetch stops the shared audio engine; make sure command listening is live here.
            viewModel.startGuidanceVoiceCommands()
        }
        .onChange(of: viewModel.isCleanFlowDestinationRecognized) { _, recognized in
            // When the destination resolves, keep the recognizer alive for "next"/"edit".
            if recognized {
                viewModel.startGuidanceVoiceCommands()
            }
        }
        .onChange(of: viewModel.pendingGuidanceCommand) { _, command in
            ...
            case .next, .resume:
                if canContinue {
                    viewModel.speakImmediate("Opening arrival preview.")
                    onConfirm()
                } else {
                    viewModel.speakImmediate("Still resolving the destination. Please wait a moment.")
                }
            case .back:
                viewModel.speakImmediate("Editing destination.")
                onEdit()
            case .repeatCue:
                viewModel.speakImmediate(spokenStatus)
            case .pause:
                break
            }
        }
    ...
    private var spokenStatus: String {
        if canContinue {
            return "Destination confirmed: \(destinationName). Say next or proceed to continue, or say edit to change it."
        }
        if isResolving {
            return "Still resolving \(destinationName). Please wait."
        }
        return "Destination not confirmed. Say edit to try a different name or address."
    }
