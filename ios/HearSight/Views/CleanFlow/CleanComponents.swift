...
struct CleanFlowBriefing {
    ...
    private static func arrivalFocus(landmarks: [String], entranceCue: String) -> String {
        let cleaned = landmarks.prefix(4).map { $0.trimmingCharacters(in: CharacterSet(charactersIn: ". ")) }
        let landmarkText = cleaned.isEmpty
            ? "What to expect near arrival"
            : "Near arrival, look for: \(cleaned.joined(separator: ", "))"
...
struct CleanScreenHeader: View {
    ...
    var body: some View {
        VStack(alignment: stackAlignment, spacing: HearSightTheme.Spacing.xs) {
            Text(title)
                // Scales with the user's Dynamic Type setting (capped so very large text still fits).
        ...
        .onAppear {
            // Steer VoiceOver focus to the screen title when the screen appears.
